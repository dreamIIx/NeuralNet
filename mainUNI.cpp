...
void mainA(sf::RenderWindow&, ::std::vector<Object>&, ::std::vector<::std::vector<Wall>>&, ::std::vector<int>&, sf::Texture&, volatile unsigned int&,
	volatile bool&, volatile bool&, volatile bool&, volatile bool&);
...
int main()
{
	...
	volatile unsigned int TIMESET_ = 0;
	...
	volatile bool newA = false;
	volatile bool auto_move = true;
...
(
	...
	sf::Texture& t,
	volatile unsigned int& TIMESET,
	volatile bool& is_Open,
	volatile bool& run_,
	volatile bool& newA_,
	volatile bool& auto_move
)
{
	win.setActive(true);
	sf::View view;
	...
	while (is_Open)
	{
		...
		if (_ptr.size() && (((!auto_move) && (!newA_)) || (auto_move && (((bufsizeO == _ptr.size()) ? *tm < TIMESET : tm.restart()) || (TIMESET == 0xFFFFFFFF)))))
		{
			bufsizeO = _ptr.size();
			for (size_t i = 0; i < _ptr.size(); ++i)
			{
